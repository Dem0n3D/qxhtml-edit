...
Base_Edit::Base_Edit()
{
    ...
    connect( wysiwyg_sroll, SIGNAL(valueChanged(int)), this, SLOT(RamPosition_wysiwyg(int)));
    connect( textareatext, SIGNAL(valueChanged(int)), this, SLOT(RamPosition_source(int)));
    connect( stext, SIGNAL(textChanged()), this, SLOT( UpdateSourceLineNr() ));
    connect(tabWidget, SIGNAL(currentChanged(int)), this, SLOT(SwapModeView(int)));
...
void Base_Edit::SwaptoSource()
{
    ModusView(true);
}

void Base_Edit::SwaptoEdit()
{
    ModusView(false);
}
...
void Base_Edit::ShowModusPanelView()
{
    ...
    BaseButton_Switsch();
   /////////// qDebug() << "### ed html   " << html;
    /////////////qDebug() << "### ed modus   editrun->" << editrun << " sourcerun->" << sourcerun;
    tabWidget->setCurrentIndex(0);
...
void Base_Edit::ShowModusPanelSourceView()
{
    ...
    ///////////qDebug() << "### so html   " << html;
    /////////////qDebug() << "### so modus   editrun->" << editrun << " sourcerun->" << sourcerun;
...
void Base_Edit::Refresh_Source()
{
    ...
    if (editrun) {
           ...
           ////////////// qDebug() << "### p." << passage << " totahtml   0=" << totahtml << " 1=" << totahtml1; 
    ...
    ////////////////qDebug() << "### 1p." << passage << " htmlsize   base=" << html.size();
    if (sourcerun) {
    html = stext->toPlainText();
    }
    ////////////qDebug() << "### 2p." << passage << " htmlsize   base=" << html.size(); 
    } else {
    /////////////////qDebug() << "### doc vuoto ...#################################################";  
    }
    
    
    //////////////////qDebug() << "### Refresh_Source end   htmlsize->"  <<   html.size()    <<     " editrun->" << editrun << " sourcerun->" << sourcerun;
...
void Base_Edit::Grabdata(const QMimeData *data)
{
  ...
  if (!editrun) {
     QMessageBox::warning(this, tr("Error"),tr("Pleas Drag on edit modus... on View Modus is not possibel to insert items!"));
  return;      
  }
   ...
   if ( data->hasImage() ) {
    numeroimmagine++;
      ...
      const  QString nuovaimmagine = QString("%2image_%1.png").arg(numeroimmagine).arg(WORK_CACHEDIR);
   ...
   if ( data->formats().contains("text/html") ) {
    ...
    if ( data->formats().contains("text/plain") ) {
             ...
             QTextDocumentFragment fragment = QTextDocumentFragment::fromHtml(QString("<p>%1</p>").arg(draghtml));
             wtext->textCursor().insertFragment(fragment);
    return;
    }
...
#endif
#if defined Q_WS_WIN
/////////qDebug() << "### wait on save time time..........  " << urlmixed.size();
QTimer::singleShot(300, this, SLOT(Reload_Editor()));   /* wait to write ready image on disk cache */
#endif
#if defined Q_WS_X11
Reload_Editor();
#endif
...
QString fileimg = QFileDialog::getOpenFileName( this, tr( "Choose Image" ), QString(setter.value("LastDir").toString()) ,tr("Image Files supported (*)"));
    ...
    if ( fileimg.isEmpty() ) {
     ...
     if (base.isNull() || !image_extension(fileimg) ) {
      QMessageBox::warning(this, tr("Image Plugin"),tr("Pleas install the plugin for this format.. or select other..\nOr copy and paste image from your images application. \nTo convert on web image. png jpg gif"));
...
void  Base_Edit::CreateanewTable()
{
    ...
    int largo = QInputDialog::getInteger(this, tr("New Table percent on page"),tr("Percent \%:"), 100, 10, 100, 1, &ok);
